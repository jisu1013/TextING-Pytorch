...
try:
    ...
    window_size = 3
    print('using 3 window size')

try:
    weighted_graph = bool(sys.argv[2])
except:
    weighted_graph = False
    print('using unweighted graph')

truncate = False # whether to truncate long documendt
MAX_TRUNC_LEN = 350
...
with open('./data/corpus/naver.clean.train.txt', 'rb') as f:
    for line in f.readlines():
...
#load raw text
doc_content_list = []
with open('./data/corpus/naver.clean.txt', 'rb') as f:
    for line in f.readlines():
